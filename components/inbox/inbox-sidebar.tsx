...
export default function InboxSidebar({ config }: InboxSidebarProps) {
  ...
  return (
    <div className="flex w-[320px] md:w-[340px] xl:w-[360px] shrink-0 flex-col border-r border-white/5 bg-[#0d1726]">
      <div className="border-b border-white/5 px-4 md:px-5 py-5">
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
          <button className={`rounded-full px-4 py-2 text-xs font-semibold whitespace-nowrap ${accentClass}`}>
            Tümü
          </button>
          <button className="rounded-full bg-white/5 px-4 py-2 text-xs font-medium text-zinc-300 whitespace-nowrap">
            Okunmamış
          </button>
          <button className="rounded-full bg-white/5 px-4 py-2 text-xs font-medium text-zinc-300 whitespace-nowrap">
            Etiketli
          ...
          {config.chats.map((chat) => (
            <div
              ...
              className={`cursor-pointer rounded-2xl border p-3 transition ${
                chat.active ? activeCardClass : "border-transparent hover:bg-white/5"
              }`}
            >
