...
import { mailFolders, mailItems, mailLabels, openedMail } from "./mail-data";
...
export default function MailPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] overflow-hidden bg-[#08111d] text-white">
      ...
      <div className={`${mobileView === "list" ? "flex" : "hidden"} w-full md:flex md:w-[380px] shrink-0 flex-col border-r border-white/5 bg-[#0f1724]`}>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <p className={`truncate text-sm ${item.unread ? "font-semibold text-white" : "text-zinc-200"}`}>
                          {item.fromName}
                        </p>
                        {item.starred ? (
                      ...
                      <p className={`mt-1 truncate text-sm ${item.unread ? "font-medium text-zinc-100" : "text-zinc-300"}`}>
                        {item.subject}
                      </p>
      ...
      <div className={`${mobileView === "detail" ? "flex" : "hidden"} min-w-0 flex-1 flex-col bg-[#07111f] md:flex`}>
        <div className="flex h-[78px] items-center justify-between border-b border-white/5 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] px-4 md:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="rounded-xl bg-white/5 p-2 text-zinc-300"><Archive size={18} /></button>
            <button className="rounded-xl bg-white/5 p-2 text-zinc-300"><Trash2 size={18} /></button>
            <button className="rounded-xl bg-white/5 p-2 text-zinc-300"><MoreHorizontal size={18} /></button>
          </div>
        </div>
        ...
        <div className="border-t border-white/5 bg-[#0d1726] px-4 py-4 md:px-6">
          <div className="mb-3 flex flex-wrap gap-2">
            <button className="rounded-full bg-white/5 px-3 py-1.5 text-xs text-zinc-300">Hazır yanıt</button>
            <button className="rounded-full bg-white/5 px-3 py-1.5 text-xs text-zinc-300">Dosya ekle</button>
            <button className="rounded-full bg-white/5 px-3 py-1.5 text-xs text-zinc-300">Etiketle</button>
          </div>
      ...
      {mobileMetaOpen ? (
        <div className="fixed inset-0 z-50 xl:hidden">
          <div className="absolute inset-0 bg-black/60" onClick={() => setMobileMetaOpen(false)} />
    ...
    <div className="flex-1 overflow-y-auto p-5">
      <div className="space-y-4">
        ...
        <div className="rounded-3xl border border-white/5 bg-white/5 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <span className="rounded-full bg-cyan-500/10 px-3 py-1 text-xs text-cyan-300">Teklif</span>
            <span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs text-emerald-300">Randevu</span>
            <span className="rounded-full bg-amber-500/10 px-3 py-1 text-xs text-amber-300">Sıcak lead</span>
          </div>
        </div>
